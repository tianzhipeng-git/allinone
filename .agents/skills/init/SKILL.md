...
3. **Package Manager**: Which package manager? (`npm`, `bun`, or `pnpm`) — default is `pnpm`
...
4. **Update `index.html`**: Set `<title>` tag
5. **Update `AGENTS.md`**: Update the Overview section with app name and description
6. **Update `README.md`**: Replace template references with app name, update description
...
11. **Update `docs/CONTRIBUTING.md`**:
    - Replace the repository placeholder with their GitHub username and repo name
...
If they chose the current project package manager, skip this step.
